  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-4 px-6 py-3">
        ...
        <form onSubmit={submit} className="order-3 flex w-full items-center gap-2 sm:order-2 sm:w-auto sm:flex-1">
          <input
            ...
            className="w-full rounded-full border border-white/10 bg-[#141414] px-5 py-2.5 text-sm text-[#f4f4f3] placeholder:text-[#6f6f72] outline-none focus:border-[#ff5c1a]"
          />
          <button
            type="submit"
            className="ca-btn-primary shrink-0 whitespace-nowrap rounded-full bg-[#ff5c1a] px-5 py-2.5 text-sm font-semibold text-[#0a0a0a]"
          >
            검색
          </button>
        </form>
        ...
        <div className="order-2 ml-auto flex items-center gap-2 sm:order-3 sm:ml-0">
          <Link
            href="/cart"
            className="ca-link-muted relative inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-white/10 px-4 py-2 text-sm text-[#c8c8cc]"
          >
            장바구니
            {count > 0 && (
              <span className="ca-mono inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[#ff5c1a] px-1.5 text-[11px] font-bold text-[#0a0a0a]">
                {count}
              </span>
            )}
          </Link>
      ...
      <nav className="mx-auto flex max-w-6xl flex-wrap gap-x-5 gap-y-1 px-6 pb-2 text-sm">
        <Link href="/" className="ca-link-muted text-[#9a9a9d]">
          홈
        </Link>
        {CATEGORIES.map((c) => (
          <Link key={c.slug} href={`/search?cat=${c.slug}`} className="ca-link-muted whitespace-nowrap text-[#9a9a9d]">
            {c.label}
          </Link>
        ))}
      </nav>
