  ...
  return (
    <section className="mt-12 border-t border-white/[0.07] pt-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-bold text-[#fafafa]">리뷰 요약 · 장단점 분석</h2>
        {!data && (
          <button
            onClick={load}
            disabled={loading}
            className="ca-btn-primary whitespace-nowrap rounded-full bg-[#ff5c1a] px-4 py-2 text-sm font-semibold text-[#0a0a0a] disabled:opacity-50"
          >
        ...
        <div className="mt-4 space-y-4">
          <div className="rounded-2xl border border-white/[0.07] bg-[#141414] p-5">
            <div className="mb-2 flex items-center gap-2">
              {data.score != null && (
                <span className="rounded-full bg-[#ff5c1a]/15 px-2 py-0.5 text-xs font-medium text-[#ff7a3d]">{data.score}점</span>
              )}
              {data.summary && <p className="text-sm text-[#c8c8cc]">{data.summary}</p>}
            </div>
            ...
            {data.verdict && <p className="mt-4 rounded-xl bg-white/[0.04] px-4 py-3 text-sm text-[#c8c8cc]">{data.verdict}</p>}
          </div>
                ...
                {data.reviews.map((r, i) => (
                  <div key={i} className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-[#ff7a3d]">
                        {"★".repeat(r.rating)}
