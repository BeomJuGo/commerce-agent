...
// 지연이 중요한 구조화 작업(추천 랭킹 등)용 빠른 비추론 모델
...
async function callOnce(messages, { model, temperature, maxTokens, timeoutMs, reasoningEffort }) {
  ...
  // 추론 모델은 reasoning_effort를 낮춰 응답 속도 확보(구조화 작업엔 low면 충분)
  if (isReasoningModel(model)) body.reasoning_effort = reasoningEffort || "low";
