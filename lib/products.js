...
// 여러 키워드 검색을 병합한 다양한 리스팅 (카테고리 진열용). TTL 캐시로 쿼터 보호.
export async function getMergedListing(key, queries, { perQuery = 4, ttlMs = 30 * 60 * 1000 } = {}) {
  const database = await db();
  if (database) {
  ...
  for (const q of queries) {
    const items = await searchShop(q, { display: perQuery }).catch(() => []);
