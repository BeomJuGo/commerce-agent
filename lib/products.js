...
import { searchShop, MIN_PRICE, dedupListings } from "./naver";
...
export async function getMergedListing(key, queries, { perQuery = 4, pages = 1, sort = "sim", ttlMs = 30 * 60 * 1000 } = {}) {
  ...
  for (const items of results) {
    for (const it of items) {
      ...
      seen.add(k);
      byKey.push({ ...it, pkey: k });
    }
  }
  // 같은 상품의 다른 리스팅(productId 상이) 중복 제거
  const merged = dedupListings(byKey);
...
export async function getListing(key, query, { display = 12, sort = "sim", ttlMs = 30 * 60 * 1000 } = {}) {
  ...
  // pkey를 미리 부여해 캐시/타일 링크에서 일관 사용 + 중복 리스팅 제거
  const withKeys = dedupListings(items.map((it) => ({ ...it, pkey: productKey(it) })));
  await cacheProducts(withKeys);
